      ...
      <div class="card card-custom mb-5">
          ...
          <div class="card-header flex-wrap border-0 pb-0 bg-light-danger">
            <h3 class="card-title w-100 d-flex justify-content-between">
              ...
              <router-link 
                to="/admin-roles/crear"
                v-if="can('roles-create')"
              class="btn btn-primary d-flex align-items-center"
            >
              <i class="flaticon-plus"></i> 
              <span class="d-none d-md-block"> Registrar Rol </span>
              </router-link>
            </h3>
          </div> 
        ...
        <div class="card-body">
         ...
         <tabla-component :striped="true"
            ...
            :listado="records" 
            titulo="Roles"     
            :can_ver="can('roles-read')"
            :can_editar="can('roles-update')"
            :can_eliminar="can('roles-delete')"       
...
<script>
    ...
    export default {
        data() {
          return {
            fields: [ 
              { key: 'title', label: 'Nombre', sortable: true, sortDirection: 'desc' },              
              { key: 'description', label: 'Descripcion', sortable: true, sortDirection: 'desc' },
              {
                key: "active",
                label: "Estatus",
                sortable: true,
        ...
        methods: { 
         ...
         ButtonGo(data) {
            this.$router.push({
                path: '/admin-roles/consultar/'+data.data.id
            });
          },    
          ButtonDelete(data) {
            this.Borrar(data);
            ...
            var registro = data.data;
             ...
             Swal.fire({
                ...
                if (result.value) {          
                    ...
                    var url = '/admin/roles/'+registro.id;
