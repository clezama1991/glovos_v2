...
class PilotosOldController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $query->when($search!='0', function($querys) use($search){ 
             $querys->where(DB::raw("CONCAT(`nombres`, ' ', `apellidos`)"), 'LIKE', "%".$search."%")
        ...
        return response(['data' => $records,'real_now'=>$real_now,'ago_3_years'=>$ago_3_years]);
